...
// Mock data for stage managers
const mockStageManagers = Array.from({ length: 24 }, (_, i) => ({
  id: i + 1,
  name: `JAMIE CHEN ${i + 1}`,
  specialty: i % 3 === 0 ? "LIVE EVENTS" : i % 3 === 1 ? "FESTIVALS" : "THEATER",
  location: i % 4 === 0 ? "LOS ANGELES" : i % 4 === 1 ? "NEW YORK" : i % 4 === 2 ? "MIAMI" : "CHICAGO",
  experience: i % 3 === 0 ? "7+ YEARS EXP" : i % 3 === 1 ? "12+ YEARS EXP" : "15+ YEARS EXP",
  rating: (4 + (i % 10) / 10).toFixed(1),
}))

export default function ViewAllStageManagersPage() {
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 12
  const totalPages = Math.ceil(mockStageManagers.length / itemsPerPage)

  const currentManagers = mockStageManagers.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
      ...
      <section className="relative pt-32 pb-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            {/* Sidebar */}
            <FilterSidebar type="stage" className="w-full lg:w-72 shrink-0" />

            {/* Main content */}
            <div className="flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentManagers.map((manager) => (
                  <ScrollReveal key={manager.id} animation="fade" delay={(manager.id * 50) % 300}>
                    <Card className="bg-white/5 backdrop-blur-sm border-white/10 overflow-hidden group hover:border-ethr-neonblue/30 transition-all duration-300">
                      <div className="aspect-square relative overflow-hidden">
                        <Image
                          src={`/placeholder.svg?key=xj60e&height=400&width=400&text=Stage`}
                          alt={manager.name}
                          width={400}
                          height={400}
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-4">
                          <div>
                            <h3 className="font-light text-lg text-white">{manager.name}</h3>
                            ...
                            <div className="flex items-center mt-1">
                              <div className="flex">
                                {[1, 2, 3, 4, 5].map((star) => (
                                  <Star
                                    key={star}
                                    className={`h-4 w-4 ${
                                      star <= Math.floor(Number.parseFloat(manager.rating))
                                        ? "text-ethr-neonblue fill-ethr-neonblue"
                                        : star - 0.5 <= Number.parseFloat(manager.rating)
                              ...
                              <span className="text-sm ml-1 text-white/70 font-light">{manager.rating}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      <CardContent className="p-4">
                        <div className="flex justify-between items-center">
                          ...
                          <Button
                            ...
                            asChild
                          >
                            <Link href={`/production/stage/${manager.id}`}>VIEW PROFILE</Link>
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
